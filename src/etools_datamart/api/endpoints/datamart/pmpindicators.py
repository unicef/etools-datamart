...
class PMPIndicatorsSerializer(DataMartSerializer):
    ...
    def get_partner_link(self, obj):
        return "%s/pmp/partners/%s/details/?schema=%s" % (config.ETOOLS_ADDRESS, obj.partner_id, obj.schema_name)

    def get_intervention_link(self, obj):
        return "%s/pmp/interventions/%s/details/?schema=%s" % (
...
class PMPIndicatorFilterForm(forms.Form):
    partner_type__in = Select2MultipleChoiceField(
        label="Partner Type", choices=PartnerOrganizationConst.CSO_TYPES, required=False
    )

    pd_ssfa_status__in = Select2MultipleChoiceField(
        label="Status", choices=PartnersInterventionConst.STATUSES, required=False
    )

    last_modify_date__gte = DatePickerField(label="Modified after", required=False)
...
class PMPIndicatorsViewSet(common.DataMartViewSet):
    ...
    serializers_fieldsets = {
        ...
        "ssfa": ["pd_ssfa_ref", "pd_ssfa_status", "pd_ssfa_start_date", "pd_ssfa_creation_date", "pd_ssfa_end_date"],
    }
    ...
    def get_querystringfilter_form(self, request, filter):
        fields = OrderedDict(
            [
                ...
                if name not in ("partner_type", "pd_ssfa_status", "last_modify_date")
            ]
        )

        return type(str("%sForm" % self.__class__.__name__), (PMPIndicatorFilterForm,), fields)(
            request.GET, filter.form_prefix
        )
