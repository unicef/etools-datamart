...
class TPMActivityFilterForm(forms.Form):
    ...
    is_pv = Select2ChoiceField(choices=(("-", "All"), (1, "True"), (0, "False")), required=False)
    visit_status__in = Select2MultipleChoiceField(label="Status", choices=TpmTpmvisitConst.STATUSES, required=False)
    date = DateRangePickerField(label="Date", required=False)

    visit_start_date = DateRangePickerField(label="Visit Start Date", required=False)
...
class TPMActivitySerializer(DataMartSerializer):
    ...
    def get_visit_url(self, obj):
        return "%s/%s?schema=%s" % (config.ETOOLS_ADDRESS, obj.visit_url, obj.schema_name)
