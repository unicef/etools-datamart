...
class TravelFilterForm(forms.Form):
    status__in = Select2MultipleChoiceField(label="Status", choices=T2FTravelConsts.CHOICES, required=False)
    ...
    approved_at = DateRangePickerField(label="Approved between", required=False)

    completed_at = DateRangePickerField(label="Completed between", required=False)
...
class TravelViewSet(common.DataMartViewSet):
    ...
    filter_fields = ("status", "created", "last_modify_date", "completed_at", "approved_at", "end_date", "start_date")
