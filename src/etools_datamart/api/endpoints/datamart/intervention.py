...
class InterventionFilterForm(forms.Form):
    status__in = Select2MultipleChoiceField(label="Status", choices=PartnersInterventionConst.STATUSES, required=False)
    ...
    document_type__in = Select2MultipleChoiceField(
        label="Document Type", choices=PartnersInterventionConst.INTERVENTION_TYPES, required=False
    )
...
class InterventionSerializerV2(DataMartSerializer):
    ...
    def get_pd_ssfa_url(self, obj):
        return "%s/pmp/interventions/%s/details/?schema=%s" % (config.ETOOLS_ADDRESS, obj.source_id, obj.schema_name)
