...
class TravelActivityFilterForm(forms.Form):
    travel_reference_number__istartswith = forms.CharField(label="Reference Number", required=False)
    ...
    primary_traveler__istartswith = forms.CharField(label="Primary Traveler", required=False)
    date = DateRangePickerField(label="Date between", required=False)
...
class TravelActivityViewSet(common.DataMartViewSet):
    ...
    filter_fields = ("travel_reference_number", "travel_type", "primary_traveler", "result_type", "date")
    ordering_fields = (
        "id",
        "created",
    )
