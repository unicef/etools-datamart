...
class PartnerAgreementSerializer(DataMartSerializer):
    ...
    def get_agreement_url(self, obj):
        return "%s/pmp/agreements/%s/details?schema=%s" % (config.ETOOLS_ADDRESS, obj.source_id, obj.schema_name)


class AgreementFilterForm(forms.Form):
    agreement_type__in = Select2MultipleChoiceField(
        label="Agreement Type", choices=PartnersAgreementConst.AGREEMENT_TYPES, required=False
    )
    status__in = Select2MultipleChoiceField(
        label="Status", choices=PartnersAgreementConst.STATUS_CHOICES, required=False
    )
