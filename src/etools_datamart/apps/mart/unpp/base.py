...
class UNPPBaseManager(DataMartManager):
    pass


class UNPPDataMartModelBase(DataMartModelBase):
    loader_option_class = BaseLoaderOptions
    loader_class = DataMartModelBase


class UNPPDataMartModel(models.Model, metaclass=UNPPDataMartModelBase):
    ...
    def service(self):
        ...
        return Service.objects.get(
            source_model=ContentType.objects.get_for_model(self),
        )
