...
class Command(BaseCommand):
    args = ''
    help = "Introspects UNPP database outputs a etools_datamart.apps.unpp.models module."
    ...
    def handle_admin(self, options):
        ...
        with output_file.open('w') as output:
            ...
            for model_name in self.unpp_models:
                ...
                for model_name in self.unpp_models:
                    if model_name not in NO_API:
                        ...
                        for line in self.handle_viewset(model_name):
                            api.write("%s\n" % line)
                        line = "router.register(r'unpp/%s', api.%sViewSet)" % (model_name.lower(), model_name)
                        urls.write("%s\n" % line)
                    else:
                        if verbosity >= 1:
    ...
    def handle_inspection(self, options):
        ...
        with connection.cursor() as cursor:
            ...
            yield "# This is an auto-generated UNPP model module."
            ...
            for table_name in (options['table'] or sorted(info.name for info in table_info if info.type in types)):
                ...
                self.unpp_models.append(model_name)
                try:
                    try:
