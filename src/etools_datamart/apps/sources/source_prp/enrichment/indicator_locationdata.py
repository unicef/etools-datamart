...
def get_previous_location_data(self: IndicatorIndicatorlocationdata):
    previous_indicator_reports = IndicatorIndicatorreport.objects.exclude(id=self.indicator_report.id).filter(
        time_period_start__lt=self.indicator_report.time_period_start
    )
