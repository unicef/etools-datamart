...
PartnersIntervention.total_unicef_cash = property(
    lambda self: self.PartnersInterventionbudget_intervention.unicef_cash_local
    if hasattr(self, "PartnersInterventionbudget_intervention")
    else 0
)

PartnersIntervention.total_in_kind_amount = property(
    lambda self: self.PartnersInterventionbudget_intervention.in_kind_amount_local
    if hasattr(self, "PartnersInterventionbudget_intervention")
    else 0
)
...
PartnersIntervention.total_partner_contribution = property(
    lambda self: self.PartnersInterventionbudget_intervention.partner_contribution_local
    if hasattr(self, "PartnersInterventionbudget_intervention")
    else 0
)

PartnersIntervention.total_unicef_budget = property(lambda self: self.total_unicef_cash + self.total_in_kind_amount)
...
aliases = (
    ["partnersintervention_partners_interventionbudget_intervention_id", "planned_budget"],
    ["partnersintervention_funds_fundsreservationheader_intervention_id", "frs"],
    ["partnersintervention_partners_interventionattachment_intervention_id", "attachments"],
