    ...
    if settings.ENABLE_LIVE_STATS:
        ...
        def _update_stats(target, **extra):
            ...
            try:
                ...
                logger.error(
                    f"""Error updating {target.__class__.__name__}: {e}
{extra}
"""
                )
        ...
        # UserCounter
        userlog, isnew = UserCounter.objects.get_or_create(day=log.requested_at, user=log.user, defaults=defaults)
        if not isnew:
            _update_stats(userlog)

        # PathCounter
        pathlog, isnew = PathCounter.objects.get_or_create(day=log.requested_at, path=log.path, defaults=defaults)
        if not isnew:
            _update_stats(pathlog)

        # MonthlyCounter
        monthlog, isnew = MonthlyCounter.objects.get_or_create(day=lastMonth, user=log.user, defaults=defaults)
        if not isnew:
            _update_stats(monthlog)

        # DailyCounter
        defaults["users"] = 1
        daylog, isnew = DailyCounter.objects.get_or_create(day=log.requested_at, defaults=defaults)
        if not isnew:
            _update_stats(daylog, users=UserCounter.objects.filter(day=log.requested_at).count())
...
class StatsMiddleware(object):
    ...
    def __call__(self, request):
        ...
        if (
            ...
            and hasattr(request, "api_info")
            and config.TRACK_PATH.match(request.path)
            and not hasattr(request, "_is_preload_internal_request")
        ):
