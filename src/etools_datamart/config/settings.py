...
env = environ.Env(
    ...
    DATABASE_URL_PRP=(str, "postgis://postgres:@127.0.0.1:5432/etools_prp"),
    DEBUG=(bool, False),
...
CONSTANCE_ADDITIONAL_FIELDS = {
    ...
    # }],
    "select_group": [
        "etools_datamart.libs.constance.GroupChoiceField",
        {
            "required": False,
...
# django-sql-explorer
EXPLORER_CONNECTIONS = {"Default": "default", "eTools": "etools"}
...
SWAGGER_SETTINGS = {
    ...
    "SECURITY_DEFINITIONS": {"basic": {"type": "basic"}, "token": {"type": "basic"}},
}
...
LOGGING = {
    ...
    "formatters": {
        ...
        "verbose": {"format": "%(levelname)-8s: %(asctime)s %(name)20s %(message)s"},
        "simple": {"format": "%(levelname)-8s: %(asctime)s %(name)20s: %(funcName)s:%(lineno)s %(message)s"},
    },
    ...
    def before_send(event, hint):
        ...
        if "exc_info" in hint:
            ...
            if isinstance(exc_value, SMTPServerDisconnected):
                ...
                isinstance(exc_value, AttributeError) and str(exc_value) == "'Run' object has no attribute 'source_id'"
            ):
                return None
...
RESULTSET_BATCH_SIZE = env("RESULTSET_BATCH_SIZE")
