...
class Export(AbstractPreload):
    ...
    filename = models.CharField(max_length=100, help_text="Optional file name")
    save_as = models.BooleanField(
        default=False,
    ...
    def check_access(self, user):
        ...
        params.update({"page_size": "1", "format": self.stem})
    ...
    def get_client(self, **kwargs):
        return Client(
            HTTP_IF_NONE_MATCH=self.etag or "Not-Set",
...
def notify_new_export(sender, instance, created, **kwargs):
    if created and settings.ADMIN_EMAILS:
        ...
        html_message = (
            f"{instance.as_user} requested a new export: "
            f"<a href=https://datamart.unicef.io{export_link}>{instance.name}</a>."
        )
