...
def test_db_is_ready_error(db, monkeypatch):
    monkeypatch.setattr("sys.exit", lambda v: v)
    with mock.patch(
        "etools_datamart.apps.init.management.commands.db-isready.Command._get_cursor", side_effect=OperationalError()
    ):
        call_command("db-isready", wait=True, timeout=1, stdout=StringIO(), stderr=StringIO())
...
def test_db_is_ready_debug(db, monkeypatch):
    ...
    with mock.patch(
        ...
        call_command("db-isready", wait=True, timeout=1, debug=True, stdout=StringIO(), stderr=StringIO())
