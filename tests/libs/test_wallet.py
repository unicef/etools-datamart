...
def test_merge(monkeypatch):
    monkeypatch.setattr("etools_datamart.libs.wallet.GLOBAL_CREDENTIALS", global_filename)
    w = Wallet(wallet_filename, obfuscate=False)
    assert w.db.username == "username1"
    assert w.db.password == "password1"
...
def test_obfuscate(monkeypatch, credentials):
    monkeypatch.setattr("etools_datamart.libs.wallet.GLOBAL_CREDENTIALS", credentials)
...
def test_obfuscated(monkeypatch):
    monkeypatch.setattr("etools_datamart.libs.wallet.GLOBAL_CREDENTIALS", obfuscated_filename)
